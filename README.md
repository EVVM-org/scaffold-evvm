...
#### 📚 [Documentation](https://www.evvm.info/docs/intro) | 🌐 [Website](https://evvm.org) | 📖 [In-app docs](http://localhost:3000/docs/)
...
| **In-app docs** (`/docs`) | A pre-built Docusaurus site bundled into `public/docs/` — no second process. Covers concepts, every core contract, every frontend page, the custom services workflow, and a CLI reference. |
...
Full per-contract reference: `/docs/contracts/*` (run the frontend and visit it).
...
│   └── docs/               # Source MDX for the bundled /docs site
...
Scaffold-EVVM ships with a pre-built Docusaurus site at **`http://localhost:3000/docs/`** — open it as soon as the frontend is running. Static files live in `packages/nextjs/public/docs/` and are served directly; no extra process to start.

**For docs authors:**

```bash
npm run docs:install   # one-time install of the Docusaurus workspace
npm run docs           # HMR dev server on http://localhost:3001/docs/
# edit MDX under packages/docs/docs/
npm run docs:build     # rebuild the static bundle in public/docs/
```

The same source is hostable at `evvm.info/docs` or any static host (`packages/docs/build/`).
